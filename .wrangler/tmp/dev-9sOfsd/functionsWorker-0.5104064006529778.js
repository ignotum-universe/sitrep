...
// .wrangler/tmp/pages-etqZlw/functionsWorker-0.5104064006529778.mjs
...
var init_conflicts = __esm({
  "api/conflicts.js"() {
    init_functionsRoutes_0_9115721520266251();
...
async function scrapeAndSave(env) {
  ...
  const html3 = sanitizeParsoidArtifacts(rawHtml);
  const data2 = parseAllTables(html3);
  if (data2.length === 0) {
...
__name(scrapeAndSave, "scrapeAndSave");
async function fetchArticleHtml() {
  ...
  const html3 = json?.parse?.text;
  if (!html3) {
    throw new Error("Unexpected API response shape: missing parse.text");
  }
  return html3;
}
__name(fetchArticleHtml, "fetchArticleHtml");
function sanitizeParsoidArtifacts(html3) {
  return html3.replace(/<!--[\s\S]*?-->/g, "").replace(/\sdata-mw\s*=\s*"(?:[^"\\]|\\.)*"/g, "").replace(/\sdata-mw\s*=\s*'(?:[^'\\]|\\.)*'/g, "").replace(/\sdata-parsoid\s*=\s*"(?:[^"\\]|\\.)*"/g, "").replace(/\sdata-parsoid\s*=\s*'(?:[^'\\]|\\.)*'/g, "");
}
__name(sanitizeParsoidArtifacts, "sanitizeParsoidArtifacts");
function parseAllTables(html3) {
  const $2 = load(html3);
  const results = [];
  let currentSection = null;
  $2("h2, h3, table.wikitable").each((_, element) => {
    ...
    if (element.tagName === "table") {
      ...
      $element.find("tr").each((_2, row) => {
        const entry = parseRow($2, row, currentSection);
        if (entry) {
          results.push(entry);
        }
      });
...
__name(parseAllTables, "parseAllTables");
function parseRow($2, row, categoryLabel) {
  const cells = $2(row).find("td, th");
  if (cells.length < 5) return null;
  const name = cleanCell($2, cells[1]);
  const location = cleanLocation($2, cells[3]);
  const cumulative_deaths = cleanCell($2, cells[4]);
...
function cleanCell($2, cell) {
  const $cell = $2(cell).clone();
...
function cleanLocation($2, cell) {
  ...
  if ($tree.length) {
    ...
    $tree.find("> ul > li").each((_, li) => {
      ...
      const text3 = $li.text().replace(/\s+/g, " ").trim();
      if (text3) locations.push(text3);
    });
    return locations.join(", ");
  }
  return $cell.text().replace(/\s+/g, " ").trim();
}
__name(cleanLocation, "cleanLocation");
var WIKI_API_URL;
var USER_AGENT;
var init_scraper = __esm({
  "utils/scraper.js"() {
    init_functionsRoutes_0_9115721520266251();
    init_browser();
    WIKI_API_URL = "https://en.wikipedia.org/w/api.php?action=parse&page=List_of_ongoing_armed_conflicts&prop=text&formatversion=2&format=json";
    USER_AGENT = "ConflictTrackerBot/1.0 (https://crisis-tracker.pages.dev; contact@example.com)";
    __name2(scrapeAndSave, "scrapeAndSave");
    __name2(fetchArticleHtml, "fetchArticleHtml");
    __name2(sanitizeParsoidArtifacts, "sanitizeParsoidArtifacts");
    __name2(parseAllTables, "parseAllTables");
    __name2(parseRow, "parseRow");
    __name2(cleanCell, "cleanCell");
    __name2(cleanLocation, "cleanLocation");
  }
});
...
var routes;
var init_functionsRoutes_0_9115721520266251 = __esm({
  "../.wrangler/tmp/pages-etqZlw/functionsRoutes-0.9115721520266251.mjs"() {
    init_conflicts();
    init_countries();
    init_scrape_now();
    routes = [
...
// .wrangler/tmp/bundle-7urdEf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7urdEf/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5104064006529778.js.map
