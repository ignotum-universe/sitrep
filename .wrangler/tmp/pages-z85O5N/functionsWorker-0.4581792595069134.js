...
var init_conflicts = __esm({
  "api/conflicts.js"() {
    init_functionsRoutes_0_38691976572223785();
...
function parseAllTables(html3) {
  ...
  const results = [];
  const sections = [
    { label: "Major War", keywords: ["Major_wars", "10,000"] },
    { label: "Minor War", keywords: ["Minor_wars", "1,000"] },
    { label: "Conflict", keywords: ["Conflicts", "100"] }
  ];
  $2("table.wikitable").each((_, table) => {
    const $table = $2(table);
    const headerText = $table.find("th").text().toLowerCase();
    if (!headerText.includes("conflict") && !headerText.includes("fatalities")) return;
    let category = "Conflict";
    const prevHeading = $table.prevAll("h2, h3").first().text().toLowerCase();
    if (prevHeading.includes("10,000") || prevHeading.includes("major")) {
      category = "Major War";
    } else if (prevHeading.includes("1,000") || prevHeading.includes("minor")) {
      category = "Minor War";
    }
    $table.find("tr").each((_2, row) => {
      const entry = parseRow($2, row, category);
...
// ../.wrangler/tmp/pages-z85O5N/functionsRoutes-0.38691976572223785.mjs
var routes;
var init_functionsRoutes_0_38691976572223785 = __esm({
  "../.wrangler/tmp/pages-z85O5N/functionsRoutes-0.38691976572223785.mjs"() {
    init_conflicts();
    init_countries();
    init_scrape_now();
